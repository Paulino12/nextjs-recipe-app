...
const OneRecipe = ({ data, preview }) => {
  ...
  return (
    <article className="min-h-screen xl:px-20 py-16 relative">
      <AnimatePresence>{noSessionLoading && <Preloader />}</AnimatePresence>
            ...
            <div className="w-full px-3 md:px-8 md:w-3/4">
              ...
              <div className="flex flex-row gap-10 mt-3">
                ...
                <div className="flex items-center justify-center">
                  <Tooltip
                    title={dietaryTooltip(data?.recipe.dietary)}
                    placement="top"
                    arrow
                  >
                    <small className="rounded-2xl bg-yellow-400 px-2 font-bold text-lg">
                      {data?.recipe.dietary}
                    </small>
                  </Tooltip>
                </div>
                ...
                {data?.recipe.allergens?.map((allergen) => (
                  <div key={allergen._key}>
                    <span className="mr-1 animate-pulse">
                      <CheckIcon fontSize="sm" color="warning" />
                    </span>
                    {allergen.allergen}
                  </div>
                ))}
              ...
              {data?.recipe?.nutritionals && (
                <div className="bg-white rounded-md shadow-lg mt-2 py-1">
                  <p className="text-xs md:text-center font-semibold w-full underline">
                  ...
                  <p className="mt-1 text-xs md:text-center font-semibold w-full">
                    ...
                    {data?.recipe?.nutritionalsPer100g["kj"]} kj /{" "}
                    {data?.recipe?.nutritionalsPer100g["kcal"]} kcal
                  </p>
                </div>
              )}
            </div>
              ...
              <div>
                {data?.recipe.ingredients?.map((ingredient, index) => (
                  <div key={index} className="flex flex-row align-baseline">
                    ...
                    <div
                      className={`${
                        !ingredientGroupTitle(ingredient) && "underline"
                      }`}
                    >
                      {ingredient}
                    </div>
                  ...
                  <div id="notASubscriber">
                    ...
                    {data?.recipe?.subRecipes?.map((subRecipe, index) => (
                      <Link
                        onClick={() =>
                          !inSession &&
                          subRecipe.name.subscriber &&
                          setShowPricing(true)
                        }
                        key={index}
                        href={`${
                          !inSession && subRecipe.name.subscriber
                            ? "#notASubscriber"
                            : `/recipes/${subRecipe.name.slug}`
                        }`}
                        className={`${
                          !inSession && subRecipe.name.subscriber
                            ? "text-gray-300"
                            : ""
                        } flex flex-row align-baseline font-[500] cursor-pointer`}
                      >
                        <span className="mr-1">
                          <CheckIcon fontSize="sm" color="success" />
                        </span>
