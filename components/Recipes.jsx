  ...
  useEffect(() => {
    const access = async () => {
      ...
      if (session) {
        ...
        axios
          .get(`/api/stripe/userSubscription/${userId}`)
          .then((response) => {
            if (
              !response.data.userSubscription &&
              response.data.userAdmin !== true
            ) {
              setNotification(
                "Please subscribe to explore recipes or Sign out."
              );
          ...
          .catch((error) => {
            if (error) {
              ...
              setNotification(
                "Something went wrong, please check your connection or contact us."
              );
              setShowNotification(true);
  ...
  useEffect(() => {
      ...
      recipes
        ...
        .filter(
          (
            value,
            index,
            arr // filtering by distinct/unique values
    ...
    setOptionsCategoryNumbers(
      recipes
        ...
        .reduce((prev, curr) => {
          // create an object that count and assign to each key
  ...
  return (
    <section id="recipes" className="min-h-screen xl:px-20 py-16">
      <div className="flex flex-col items-center justify-center text-center">
        ...
        <span className="text-sm italic mt-2">View all our recipes below</span>
      </div>
      ...
      <motion.div className="w-full md:w-1/2 md:mx-auto flex items-center justify-center mb-3">
        <div>
          <Pagination
            className="w-full"
            count={count}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filteredRecipes?.length > 0 ? (
              paginatedRecipes.currentData().map((recipe, index) => (
                <motion.div
                  className="relative pb-2 w-full md:w-72 overflow-hidden shadow-lg hover:shadow-2xl"
                  key={index}
                >
                  <AnimatePresence>
                    {isLoading && recipeId === recipe._id && (
                      <Preloader framerOpacity="0.5" />
                    )}
                  </AnimatePresence>
