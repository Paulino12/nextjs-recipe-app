...
const ProductsPrices = () => {
  // initiate context
  const { isLoading, setIsLoading, setNotification, setShowNotification } =
  ...
  const handleProduct = (product) => {
    ...
    axios
      .get(
        `/api/stripe/createSubscriptionSession/${product}/${sessionData.user[2]}`
      )
      .then((response) => {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center py-20 px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full md:w-1/2 md:mx-auto h-full flex flex-col md:flex-row space-y-10 md:space-y-0 md:space-x-10 items-center md:items-start justify-center">
        <div
          onClick={() => handleProduct(standardProductPrice)}
        ...
        >
          <div className="my-6 py-1 px-2 font-semibold uppercase rounded-xl bg-yellow-400 text-yellow-800">
            Octav Recipes Standard
          </div>
